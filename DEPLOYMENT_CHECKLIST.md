...
2. **Create Organization**: Nombre: "Basileia"
3. **Create Project**: Nombre: "Basileia Production"
4. **Create Database**:
...
6. **Get Connection String**:
   ...
   - **AÑADE** `/Basileia` antes del `?`:
     ```
     mongodb+srv://Basileia_admin:PASSWORD@Basileia-prod.xxxxx.mongodb.net/Basileia?retryWrites=true&w=majority
     ```
...
3. **Add Sender** (para evitar spam):
   ...
   - "Add a Sender"
   - Email: `noreply@Basileia.com` (o tu dominio real después)
   - From name: `Basileia`
   - Click "Save"
...
heroku config:push --file .env.production --app Basileia-api
```

**Opción 2 - Manual** (si plugin falla):
```bash
heroku config:set MONGO_URL="mongodb+srv://..." --app Basileia-api
heroku config:set JWT_SECRET_KEY="n6Xz2NfbgcoQjwoaQR+..." --app Basileia-api
...
```bash
heroku addons:create heroku-redis:hobby-dev --app Basileia-api
```
...
```bash
heroku logs --tail --app Basileia-api
heroku open --app Basileia-api
```

**Test health endpoint**:
```bash
curl https://Basileia-api.herokuapp.com/health
```
...
# Cuando pregunte el valor: https://Basileia-api.herokuapp.com
```
...
2. Buscar dominio: `Basileia.app` o `Basileia.dev`
3. Checkout (usa Student Pack, $0)
4. Configurar DNS:
   - Frontend (Vercel): Settings → Domains → Add `Basileia.app`
   - Backend (Heroku): Settings → Domains → Add `api.Basileia.app`
...
# Verificar que añadiste /Basileia en la URL
```
