...
- [ ] Verificar que no existan secretos hardcodeados en Frontend/src ni backend/.
- [ ] Confirmar que SNYK_TOKEN exista en GitHub Actions (Repository secrets).

## 2) Seguridad de runtime (backend)
- [ ] Forzar CORS solo a dominios de produccion (Vercel) y staging aprobado.
...
- [ ] Validar logs sin PII en errores, especialmente tokens y correos.
...
## 4) CI/CD y calidad
- [ ] Mantener workflows en verde en main: Tests, Deploy a Produccion, Security Scan.
- [ ] Activar bloqueo de merge si backend-tests falla en PR.
- [ ] Ejecutar Security Scan semanal (schedule) y registrar hallazgos.
- [ ] Confirmar que staging no rompa pipelines cuando falten secretos (skip controlado).
