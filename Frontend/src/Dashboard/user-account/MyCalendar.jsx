...
const sourceLabel = (source) => (source === 'google' ? 'Google' : 'Medicare');

const sourceBadgeClass = (source) => {
	if (source === 'google') return 'bg-sky-50 text-sky-700 border border-sky-100';
	if (source === 'medicare') return 'bg-indigo-50 text-indigo-700 border border-indigo-100';
...
const computeExperience = (events) => {
	...
	const localCount = events.filter((evt) => evt.extendedProps?.source === 'medicare').length;
...
const MyCalendar = () => {
	...
	const localEvents = useMemo(
		() => events.filter((evt) => evt.extendedProps?.source === 'medicare'),
		[events]
	);
						...
						<div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 py-4 text-sm text-slate-600">
							<div className="flex flex-wrap gap-3">
								<div className="rounded-2xl border border-slate-200 px-3 py-2">
									<p className="text-[11px] uppercase tracking-[0.3em] text-slate-400">Medicare</p>
