            ...
            {filteredRecords.map((record) => (
              <div key={record._id} className="border-l-4 border-primaryColor bg-gray-50 rounded-r-lg p-4 hover:shadow-md transition-shadow">
                ...
                {record.attachments && record.attachments.length > 0 && (
                  <div className="mt-3">
                    <p className="text-xs text-gray-600 mb-2">Adjuntos ({record.attachments.length}):</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm text-gray-600 mb-1">Archivos (opcional, max 5MB c/u, JPG/PNG/PDF)</label>
                <input
                  type="file"
                ...
                />
                {form.files.length > 0 && (
                  <p className="text-xs text-gray-600 mt-1">{form.files.length} archivo(s) seleccionado(s)</p>
                )}
              </div>
            </div>
