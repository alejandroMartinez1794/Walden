...
const questions = [
  ...
  '¿Con qué frecuencia se sintió tan triste o deprimido que nada podía animarle?',
...
const interpret = (score) => {
  if (score >= 13) return { label: 'Alto', color: 'text-red-600' };
  if (score >= 8) return { label: 'Moderado', color: 'text-orange-600' };
...
const K6Form = () => {
    ...
    try {
      ...
      const payload = {
        ...
        interpretation: {
          ...
          notes: `K6 ${total}/24. ${severity.label} malestar psicológico.`,
        },
      };
      ...
      toast.success('K6 guardado en el expediente');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-headingColor">K6</h1>
        <p className="text-textColor mt-1">Kessler Psychological Distress Scale (6 ítems)</p>
        <p className="text-sm text-gray-500 mt-2">Últimas 4 semanas. Rango 0-24 (0-4 por ítem). Corte sugerido ≥13.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6">
          <p className="text-blue-800 text-sm">
            <strong>Consentimiento:</strong> Medida breve de malestar psicológico. No es diagnóstico; interpretar junto a la clínica.
          </p>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-2xl font-bold">{total} / 24</p>
          </div>
          <div>
        ...
        >
          {submitting ? 'Guardando...' : 'Guardar K6'}
        </button>
      </form>
    </div>
  );
