// Frontend/src/Dashboard/psychology/assessments/BAI/BAIForm.jsx
...
const BAIForm = () => {
  ...
  const severity = total >= 26 ? 'Severa' : total >= 16 ? 'Moderada' : total >= 8 ? 'Leve' : 'Mínima';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        scores: { total },
        interpretation: { severity: severity.toLowerCase(), notes: `BAI: ${total}/63 (${severity})` },
      };
      ...
      toast.success('Evaluación BAI guardada');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-headingColor">BAI</h1>
        <p className="text-textColor mt-1">Beck Anxiety Inventory</p>
      </div>
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6 mb-6 text-center">
          ...
          <p className="text-5xl font-bold text-primaryColor mb-2">{total}<span className="text-2xl text-gray-500">/63</span></p>
          <p className="text-lg font-semibold">Ansiedad {severity}</p>
        </div>
